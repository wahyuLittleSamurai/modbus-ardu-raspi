...
void loop() 
{
  ...
  */
  ...
  int valueTemperature = temperature4/2;  //cari rata2 sensor
  int valueHumidity = humidity4/2;
...
void readDHT(boolean show)  //function baca DHT
{
  ...
  valDhtH[1] = dht4.readHumidity();

  valDhtC[0] = dht1.readTemperature();
  valDhtC[1] = dht4.readTemperature();
  if(show)
  {
    lcd.setCursor(0,0);
    sprintf(lcdBuff,"%3d %3d",valDhtH[0],valDhtH[1]);
    lcd.print(lcdBuff);
    lcd.setCursor(0,1);
    sprintf(lcdBuff,"%3d %3d",valDhtC[0],valDhtC[1]);
    lcd.print(lcdBuff);
  }
}
